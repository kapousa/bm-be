...
class FaceDetectionCotroller:
    ...
    def create_model(self, ds_goal, ds_source):
        return self._create_face_detecting_model(ds_goal, ds_source)

    def _create_face_detecting_model(self, ds_goal, ds_source):
        try:
    ...
    def recognizefaces(self, run_identifier, desc, host, uname, pword, channel):
        return self._detect_faces(run_identifier, desc, host, uname, pword, channel)

    def _recognize_faces(self, run_identifier, desc, host, uname, pword, channel): # Run identifier is combination from model_id_run_id
        try:
            # Label files
